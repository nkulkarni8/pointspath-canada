...
try:
    ...
    print("🗑️  Deleting existing routes...")
    db.execute(text("DELETE FROM routes"))
    db.commit()
    print("✅ Deleted old routes")
    ...
    print(f"📥 Inserting {len(routes_data)} routes...")
    ...
    for i, route_data in enumerate(routes_data, 1):
        route = Route(
            ...
            program=route_data[8]
        )
        db.add(route)
        
        if i % 50 == 0:
            print(f"  Inserted {i}/{len(routes_data)} routes...")
    
    db.commit()
    
    # Verify
    count = db.query(Route).count()
    print(f"\n✅ Successfully inserted {count} routes!")
    print(f"🎉 Database updated with all bidirectional routes!")
